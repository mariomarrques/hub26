...
export function CategoryCard({ category, index = 0, canManage, onEdit, onDelete }: CategoryCardProps) {
  return (
    <Link
    ...
    >
      <div className="aspect-[4/3] overflow-hidden">
        ...
        {/* Action buttons for admins/mods */}
        {canManage && (
          <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            ...
            <Button
              size="icon"
              variant="secondary"
              className="h-8 w-8 bg-black/50 backdrop-blur-sm hover:bg-destructive text-white border-0"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                onDelete?.(category);
